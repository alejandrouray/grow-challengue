import PropTypes from 'prop-types'
import Icon from './Icon'

const Planet = ({ className = '', title, population = 0, maxSurfaceWater, surfaceWater }) => {
  const formattedPopulation = population !== 'unknown' ? new Intl.NumberFormat('en-US').format(Number(population)) : population
  const surfaceWaterWidth = 100 * (surfaceWater === 'unknown' ? 0 : Number(surfaceWater)) / maxSurfaceWater

  return (
    <div className={`${className} w-full max-w-sm bg-white rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 py-4`}>
      <div className='px-5'>
        <h5 className='text-xl pb-2 font-semibold tracking-tight text-gray-900 dark:text-white'>{title}</h5>
        <div className='grid gap-y-6 items-center'>
          <div className='flex gap-x-1'>
            <Icon filename='group.svg' title='Population' className='w-5' />
            <span className='text-1xl font-bold text-gray-900 dark:text-white'>{formattedPopulation}</span>
          </div>
          <dl>
            <dt className='text-sm font-medium text-gray-500 dark:text-gray-400'>Surface Water</dt>
            <dd className='flex items-center mb-3'>
              <div className='w-full bg-gray-200 rounded h-2.5 dark:bg-gray-700 mr-2'>
                <div
                  className='bg-blue-600 h-2.5 rounded dark:bg-blue-500'
                  style={{ width: `${surfaceWaterWidth}%` }}
                />
              </div>
              <span className='text-sm font-medium text-gray-500 dark:text-gray-400'>{surfaceWater}</span>
            </dd>
          </dl>
          <a href='#' className='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>See Details</a>
        </div>
      </div>
    </div>
  )
}

Planet.propTypes = {
  className: PropTypes.string,
  title: PropTypes.string.isRequired,
  population: PropTypes.string.isRequired
}

export default Planet
